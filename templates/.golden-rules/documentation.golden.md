...
- [GR-D012] **跨域及对话输出必须使用 `file:///` 绝对物理定锚**
  → 检查方法: 凡是提供给用户点击的工作流物料（Artifacts如 实施计划、总结报告，或在聊天面板发给用户的各种超链），都绝对不能再使用仅靠当前路径拼凑的半吊子链接！必须使用 `[名称](file:///c:/完整绝对路径/名称.md)` 协议！
  → 违规信号: 在输出报告或聊天文本中看到了让人点击的 `[查看这儿](docs/a.md)` 或 `../../`。
  → 修复方式: 不要在试图从外围环境寻找它了，它已经被前端框架抛弃导致 404！一律转换为带 `file:///c:/` 等磁盘根级指派的物理强击穿链接。
  → 来源: 痛定思痛的寻址偏移血案（跨工作区点击后上溯引发的文件不存在乱象）。
