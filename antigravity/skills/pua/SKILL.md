...
## 失败沉淀闭环 — Harness 自修复循环

> **核心公式**: Agent 犯同样错误 2 次 → 检查是否需要更新 Harness

当 PUA 驱动 Agent 经过反复重试后**最终成功**时，不能直接庆祝——必须执行以下沉淀流程：

### Step 1: 失败模式分析

回答三个问题：
1. **是什么类型的失败？** — 架构违规 / 环境配置 / API 变更 / 文档过时 / 类型错误
2. **根因是什么？** — Agent 缺少信息 / Agent 误解了约束 / 工具链有 bug / 文档有歧义
3. **这个失败可预防吗？** — 如果提前知道某个信息，能否避免此次失败？

### Step 2: 沉淀判定

| 根因分类 | 沉淀目标 | 示例 |
|---------|---------|------|
| Agent 缺少项目上下文 | 更新 `AGENTS.md`（根目录或子模块级） | 新增"此模块使用延迟加载" |
| Agent 违反了架构约束 | 新增 `.golden-rules/` 准则条目 | 新增 GR-A010 |
| 底层库/工具的已知坑 | 新增 `.golden-rules/error-hints.golden.md` 条目 | 新增 AGENT_HINT |
| 文档描述有歧义 | 修正相关文档 + 更新 AGENTS.md | 修复 architecture.md 过时描述 |
| 一次性偶发问题 | 不沉淀 | 网络超时、临时环境故障 |

### Step 3: 执行沉淀

如果判定需要沉淀：

1. 在回复中用 `[HARNESS-IMPROVEMENT]` 标签标注改进内容
2. 立即更新对应的文件（AGENTS.md / .golden-rules/ / error-hints）
3. 新增的准则必须包含 `来源:` 字段，追溯到本次失败

```
[HARNESS-IMPROVEMENT]
类型: 新增黄金准则
文件: .golden-rules/architecture.golden.md
内容: GR-A010 - Fabric.js 事件监听必须在 onCanvasReady 之后注册
来源: 第 N 期开发 — canvas-engine 事件监听竞态 bug（PUA L3 才修复）
```

### Step 4: 触发 agent-eval

如果本次任务触发了 L3 或更高级别的 PUA，在任务完成后自动建议触发 `agent-eval` 技能进行质量评估。
