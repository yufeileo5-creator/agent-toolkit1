...
| `🏷️ 名称 \| ¥价格 \| ~~¥原价~~ \| 概述` | 单条商品主控行 | 商品卡片头与主副面板 |
| `- [券] 细则文本` | 商品内包含的单据/子权益 | 生成归属于上方商品的精美网格子券阵列 |
...
- [ ] 所有的权益明细**绝对不可直接揉在** `🏷️` 的权益栏！必须跨行提取出每一个小项（如："1张15元券"、"免费送3次洗车"），每一项都用 `- [券] xxx` 做跟随渲染。
...
- ❌ **权益未分裂**：把优惠券全都堆在一个横线里 → 严禁！任何含有张数、优惠项的点，必须换行使用 `- [券]` 单独列出！！
- ❌ **概括规则条款**：把 8 条细则"概括"成"详见活动规则" → 必须逐条罗列。
- ❌ **单独捏造**：跳过 Markdown 格式直接吐 json 或胡编长句 → 必须使用专属标记语言解析链路。
