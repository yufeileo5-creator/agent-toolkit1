    ...
    for entity_dir in entities_dir.iterdir():
        if entity_dir.is_dir():
            ...
            if meta_file.exists():
                with open(meta_file, "r", encoding="utf-8") as f:
                    entity = json.load(f)
                # 在 uid、source、purpose、features 中搜索
                features_str = " ".join(entity.get("features", []))
                searchable = f"{entity.get('uid', '')} {entity.get('source', '')} {entity.get('purpose', '')} {features_str}".lower()
...
def cmd_tag_feature(args) -> None:
    ...
    for uid in args.uids:
        entity = load_entity(dsp_root, uid)
        if not entity:
            print(f"⚠️  跳过: 找不到实体 {uid}", file=sys.stderr)
            continue

        if "features" not in entity:
            entity["features"] = []

        if feature_name not in entity["features"]:
            entity["features"].append(feature_name)
            save_entity(dsp_root, uid, entity)
            print(f"   ✅ {uid} [{entity.get('source', '?')}] <- 🏷️ {feature_name}")
            tagged_count += 1
        else:
            print(f"   ⏭️  {uid} 已有标签 '{feature_name}'，跳过")

    print(f"\n✅ 功能标签 '{feature_name}' 已标记到 {tagged_count} 个实体")


def cmd_untag_feature(args) -> None:
    """移除实体上的功能标签"""
    dsp_root = get_dsp_root(args.root)
    ensure_dsp_exists(dsp_root)
    ...
    if args.feature_name in features:
        features.remove(args.feature_name)
        entity["features"] = features
        save_entity(dsp_root, args.uid, entity)
        print(f"✅ 已从 {args.uid} 移除功能标签 '{args.feature_name}'")
    else:
        print(f"⚠️  实体 {args.uid} 上不存在标签 '{args.feature_name}'")


def cmd_list_features(args) -> None:
    """列出所有已注册的功能标签及其关联实体数量"""
    dsp_root = get_dsp_root(args.root)
    ensure_dsp_exists(dsp_root)

    # 收集所有功能标签 -> 关联实体列表
    feature_map = {}  # feature_name -> [entity_info]
    entities_dir = dsp_root / "entities"
    if not entities_dir.exists():
        print("图谱为空，无功能标签")
        return
    ...
    for entity_dir in entities_dir.iterdir():
        if entity_dir.is_dir():
            ...
            if meta_file.exists():
                with open(meta_file, "r", encoding="utf-8") as f:
                    entity = json.load(f)
                for feat in entity.get("features", []):
    ...
    print(f"📋 功能标签列表 ({len(feature_map)} 个功能):\n")
    for feat_name in sorted(feature_map.keys()):
        entities = feature_map[feat_name]
        print(f"  🏷️  {feat_name} ({len(entities)} 个实体)")
        for ent in entities:
            print(f"      └── {ent['uid']} [{ent['source']}]")


def cmd_find_by_feature(args) -> None:
    """查找关联某功能标签的所有实体"""
    dsp_root = get_dsp_root(args.root)
    ensure_dsp_exists(dsp_root)
    ...
    entities_dir = dsp_root / "entities"
    if not entities_dir.exists():
        print(f"未找到关联功能 '{feature_name}' 的实体")
        return
    ...
    for entity_dir in entities_dir.iterdir():
        if entity_dir.is_dir():
            ...
            if meta_file.exists():
                with open(meta_file, "r", encoding="utf-8") as f:
                    entity = json.load(f)
                if feature_name in entity.get("features", []):
                    results.append(entity)
    ...
    print(f"🔍 功能 '{feature_name}' 关联的实体 ({len(results)} 个):\n")
    for entity in results:
        print(f"  {entity['uid']} [{entity.get('type', '?')}:{entity.get('kind', '?')}] {entity.get('source', '?')}")
        print(f"    用途: {entity.get('purpose', '?')}")
        all_features = entity.get('features', [])
        other_features = [f for f in all_features if f != feature_name]
        if other_features:
            print(f"    其他功能标签: {', '.join(other_features)}")
...
def main():
    ...
    # tag-feature
    p = subparsers.add_parser("tag-feature", help="给实体打功能标签")
    p.add_argument("feature_name", help="功能名称")
    p.add_argument("uids", nargs="+", help="实体 UID 列表")

    # untag-feature
    p = subparsers.add_parser("untag-feature", help="移除实体上的功能标签")
    p.add_argument("uid", help="实体 UID")
    p.add_argument("feature_name", help="要移除的功能名称")

    # list-features
    subparsers.add_parser("list-features", help="列出所有功能标签")

    # find-by-feature
    p = subparsers.add_parser("find-by-feature", help="按功能标签查找实体")
    p.add_argument("feature_name", help="功能名称")
    ...
    commands = {
        ...
        "tag-feature": cmd_tag_feature,
        "untag-feature": cmd_untag_feature,
        "list-features": cmd_list_features,
        "find-by-feature": cmd_find_by_feature,
    }
